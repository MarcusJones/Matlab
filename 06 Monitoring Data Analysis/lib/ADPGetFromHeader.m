...
% this is a function used by many of the ADP processing functions to search
% in the header for colums;
%
% See also ADPRead
todisplay=[];
l=size(header);
...
for i=1:length(x)
    ...
    if isvector(current) && ~ischar(current)
        ...
        if ischar(current)
            if ~isempty(regexp(current,'^°C$|^h$|Wh$|^%$|^bar$|^A$|m[^][23]$|/s$|^m3/h$','once'))
...
% finally error if there was nothing selected
if isempty(out)
    error('No valid Sensors where selected');
end
