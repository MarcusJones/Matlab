...
%set(fig, 'PaperSize', [29.677 20.984]);
...
% Wim> GhostScript is a set of tools that can be used in command line (in batch) under all OS. There is also a graphical interface.
%
...
if status
    ...
    if 0 %isempty(strfind(result, 'not recognized'))
        fprintf('%s\n', result);
    else
        ...
        if 0 %isempty(cmd)
            ...
            cmd = ['D:\Apps\gs\gs8.71\bin\gswin32c.exe'];
            system([cmd options]);
        end
    end
end
...
if ispc
    ...
    default_location = 'D:\Apps\gs\gs8.71';
...
for a = 1:numel(dir_list)
    ...
    if ~isempty(ver_num2) && ver_num2 > ver_num
        ...
        cmd2 = ['D:\Apps\gs\gs8.71\bin\gswin32c.exe']
...
new_style = {'/dom { dpi2point 1 currentlinewidth 0.08 mul add mul mul } bdef',... % Dot length macro based on line width
    ...
    '/GR { [1 dom 1 dom] 1 setdash 1 setlinecap } bdef'}; % Dotted lines
