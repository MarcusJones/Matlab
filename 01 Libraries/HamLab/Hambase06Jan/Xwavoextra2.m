...
% Riw warmteweerstand aan de binnenzijde van glas naar binnen als gevolg van de zonwering,Rew aan de
% buitenzijde (voor condensatieberekeningen)  
 
if isfield(BASE,'Glas')
    kglas=find(cellfun('isempty',BASE.Glas)==0);
    for i=kglas
        InBuil.glas.Riwa(i)=1/BASE.Glas{i}(6)-1/BASE.Glas{i}(1)+InBuil.glas.Ri(i); %binnenzonwering
        InBuil.glas.Rewa(i)=0*(1/BASE.Glas{i}(6)-1/BASE.Glas{i}(1))+InBuil.glas.Re(i);;%buitenzonwering
    end 
else
    InBuil.glas.Riwa(1)=0.13;
    InBuil.glas.Rewa(1)=0.04;
end
