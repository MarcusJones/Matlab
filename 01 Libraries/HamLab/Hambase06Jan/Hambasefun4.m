...
%
% usage  [Control,Profiles,InClimate,InBuil]=Hambasefun3(BASE),
%
...
% dec2004/sept2005 Riglas=8.29 regel 305
%
%
function [Control,Profiles,InClimate,InBuil]=Hambasefun4(BASE)
% ------------------------------------------------------------------------
%
%                Warmte en vocht
...
for i=1:length(zone1)
    ...
    if BASE.Vol{k}~=0
        kzone=kzone+1;
        zone(kzone)=k;
        vol(kzone)=BASE.Vol{k};
        Qpmax(kzone)=BASE.Plant{k}(1);
        Qcmax(kzone)=BASE.Plant{k}(2);
        Gmax(kzone)=BASE.Plant{k}(3);
        Gmin(kzone)=BASE.Plant{k}(4);
        CFset(kzone)=BASE.convfac{k}(2);
        CFint(kzone)=BASE.convfac{k}(3);
        CFh(kzone)=BASE.convfac{k}(1);
        etaww(kzone)=BASE.heatexch{k}(1);
        Twws(kzone)=BASE.heatexch{k}(2);
        fbv(kzone)=BASE.furnishings{k}(1);
        CFfbi(kzone)=BASE.furnishings{k}(2);
        wfun(kzone,:)=BASE.weekfun{k};
...
InClimate.gref=gref;
InClimate.LAT=LAT;
InClimate.LSMLON=LSMLON;
InClimate.nin=nin;

eval(['load mt',num2str(jaar)]);
eval(['kli1=mt',num2str(jaar),'(:,[1 2 3 9 6]);']);
eval(['clear mt',num2str(jaar)]);
date1(1:length(kli1(:,1)))=jaar;
...
for id=1:aantaldagen
    ...
    %wkfun(zone) Profiles nr op een bepaalde dag in de week
    %Van de invoer Profiles (per dag , weekfun!, bijv. 3 perioden)wordt een uurfile gemaakt
