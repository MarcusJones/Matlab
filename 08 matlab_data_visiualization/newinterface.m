...
datacursorstarterr=uitoggletool(tools,'Cdata',d.cdata,'ClickedCallback',@freshdatafunction);
...
readData(0,0);
    function output_txt=freshdatafunction(obj,event_obj)
        ...
        if strcmp(get(datacursorstarterr,'State'),'on')
            set(handler,'Enable','on');
            current=figure;
            set(current,'Menubar','figure');
        else
        ...
        function out_text=datasearch(obj,event_obj)
            ...
            out_text=num2str(idx);
            datacursormode off
            set(current,'CurrentObject',mainfig);
        end
    end
